...
public class SwipeBackLayout extends ViewGroup {
    ...
    private void ensureTarget() {
        if (target == null) {
            if (getChildCount() > 1) {
                throw new IllegalStateException("SwipeBackLayout must contains only one direct " +
                        "child");
            }
            target = getChildAt(0);
    ...
    private void findScrollView(ViewGroup viewGroup) {
        ...
        if (viewGroup.getChildCount() > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (child instanceof AbsListView || child instanceof ScrollView || child
                        instanceof ViewPager || child instanceof WebView) {
                    scrollChild = child;
                    return;
                }
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (getChildCount() > 0) {
            int measureWidth = MeasureSpec.makeMeasureSpec(getMeasuredWidth() - getPaddingLeft()
                    - getPaddingRight(), MeasureSpec.EXACTLY);
            int measureHeight = MeasureSpec.makeMeasureSpec(getMeasuredHeight() - getPaddingTop()
    ...
    private class ViewDragHelperCallBack extends ViewDragHelper.Callback {
        ...
        public void onViewDragStateChanged(int state) {
            ...
            if ((draggingState == ViewDragHelper.STATE_DRAGGING || draggingState ==
